...
<body>
	<div class="wrapper">
		...
		<div class="add">
			...
			数据库中共有&nbsp&nbsp<?php echo $num; ?>&nbsp&nbsp个调转记录&nbsp;&nbsp;&nbsp
		</div>
		<table>
			<tr>
				<th>调转编号</th>
			...
			foreach ($data as $key => $value) {
				...
				echo "<td>{$arr['原部门或大队']}</td>";
				echo "<td>{$arr['原岗位']}</td>";
				echo "<td>{$arr['分配调转部门或大队']}</td>";
				echo "<td>{$arr['分配调转后岗位']}</td>";
				echo "<td>{$arr['分配调转时间']}</td>";
				echo "<td>{$arr['分配调转原因']}</td>";
				echo "<td>{$arr['备注']}</td>";
				echo "<td>
