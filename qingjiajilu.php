...
//编写查询sql语句
$sql = 'SELECT * FROM `职工请销假`ORDER BY`职工请销假`.`递交请假条日期` DESC';
//执行查询操作、处理结果集
...
<body>
	<div class="wrapper">
		...
		<div class="add">
			...
			数据库中共有&nbsp&nbsp<?php echo $num; ?>&nbsp&nbsp个请假记录&nbsp;&nbsp;&nbsp
		</div>
		<table>
			<tr>
				<th>报告编号</th>
			...
			foreach ($data as $key => $value) {
				...
				echo "<td>{$arr['所属部门']}</td>";
				echo "<td>{$arr['所属大队']}</td>";
				echo "<td>{$arr['请假开始日期']}</td>";
				echo "<td>{$arr['请假结束日期']}</td>";
				echo "<td>{$arr['请假天数']}</td>";
				echo "<td>{$arr['递交请假条日期']}</td>";
				echo "<td>{$arr['请假原因']}</td>";
				echo "<td>{$arr['请假类别']}</td>";
				echo "<td>
						<a href='javascript:del({$arr['ID']})'></a> 
						<a href='edit_qingjia.php?id={$arr['ID']}'>修改</a>
					  </td>";
				echo "</tr>";
				// echo "<pre>";
				// print_r($arr);
